%m-file to run the mex function, mx_jCorv_LIF2Pop_delay.c, for Evoked state
...
Iap_OB=[0.9*ones(.2*length(ThresOB),1); 0.6*ones(.8*length(ThresOB),1)]; %Evoked
...
save d_evkHet nuPC nuOB mn_* var_* icov_* T_win sigp cr_pi Iap_*